# Standalone Ventus TMA V3.3 synthesis at TSMC N12 TT 1.0 V 85 C.
...
set drop_pmu [getenv_default "DROP_PMU_OUTPUTS" 0]
...
set variant [getenv_default "VARIANT" "tma_v3_3"]
...
puts "Ventus TMA V3.3 DC: $variant top=$design_name"
...
# The archived V3.2 RTL was elaborated with PMU enabled. Removing only its
# top-level perf outputs makes the otherwise-unobservable PMU cone dead before
# optimization, matching the compile-time PMU-off V3.3 comparison boundary.
if {$drop_pmu} {
  # DC represents a Verilog vector as one bus plus bit ports.  Dissolve the
  # bus first, then the former members can be removed as ordinary ports.
  remove_bus "io_perf_s2g*"
  set pmu_scalars [get_ports -quiet "io_perf_s2g*"]
  if {[sizeof_collection $pmu_scalars] > 0} {
    puts "INFO: remove [sizeof_collection $pmu_scalars] V3.2 PMU output bits"
    remove_port $pmu_scalars
  }
}
...
puts $summary "Drop_PMU_Output_Ports: $drop_pmu"
...
puts "INFO: TMA V3.3 N12 TT 1.0V 85C 1.5GHz synthesis finished"
exit
