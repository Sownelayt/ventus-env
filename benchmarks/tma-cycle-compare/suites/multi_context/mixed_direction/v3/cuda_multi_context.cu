// Independent-command scaling probe for same-direction and mixed-direction TMA.
// One binary serves two immutable suite packages; --project selects the CSV
// matrix.  All commands use disjoint global lines, shared tiles and barriers.
...
int main(int argc, char** argv) {
  ...
  if (project == "same") {
    for (int path : {kTensor, kBulk}) for (int direction : {kG2S, kS2G}) for (int bytes : {128, 4096}) for (int contexts : {1, 2, 4, 8, 16, 32}) for (int map_mode : {kSameMap, kDistinctMap}) {
      if (direction_filter != "all" && direction_filter != direction_name(direction)) continue;
      if (path == kBulk && map_mode == kDistinctMap) continue;
      for (int mode : {kSerial, kBatched}) {
        // A measurement case owns its descriptor allocation.  Keeping the
        // serial case's TensorMap for the following batched case would make
        // the latter a TMAU-hit experiment even though it is labelled cold.
        MapAllocation maps{};
        if (path == kTensor) {
          maps = make_maps(
              direction == kG2S ? source : destination, bytes, contexts,
              map_mode, maps_to_free);
          warm_maps(maps);
        }
        CUDA_CHECK(cudaMemset(destination, 0, static_cast<size_t>(kRepeats) * contexts * bytes)); CUDA_CHECK(cudaMemset(samples, 0, kRepeats * sizeof(Sample)));
        const size_t shared = static_cast<size_t>(contexts) * bytes + contexts * sizeof(Barrier);
        CUDA_CHECK(cudaFuncSetAttribute(same_cta_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, static_cast<int>(shared)));
        same_cta_kernel<<<1, kThreads, shared>>>(path, direction, mode, map_mode, contexts, bytes, maps.pointer, source, destination, samples);
        CUDA_CHECK(cudaGetLastError()); CUDA_CHECK(cudaDeviceSynchronize()); CUDA_CHECK(cudaMemcpy(host.data(), samples, kRepeats * sizeof(Sample), cudaMemcpyDeviceToHost));
        unsigned int errors = 0; if (direction == kS2G) { const size_t result_bytes = static_cast<size_t>(path == kTensor ? 1 : kRepeats) * contexts * bytes; std::vector<unsigned char> actual(result_bytes); CUDA_CHECK(cudaMemcpy(actual.data(), destination, actual.size(), cudaMemcpyDeviceToHost)); for (auto value : actual) errors += value != kValue; }
        emit("same_direction", "intra_cta", path, direction_name(direction), "na", mode, map_mode, bytes, contexts, contexts, host, errors, path == kTensor && direction == kG2S ? &maps : nullptr, path == kTensor && direction == kS2G ? &maps : nullptr);
      }
    }
    for (int path : {kTensor, kBulk}) for (int direction : {kG2S, kS2G}) for (int bytes : {128, 4096}) for (int map_mode : {kSameMap, kDistinctMap}) {
      if (direction_filter != "all" && direction_filter != direction_name(direction)) continue;
      if (path == kBulk && map_mode == kDistinctMap) continue;
      int blocks_per_sm = 0;
      CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
          &blocks_per_sm, cooperative_same_kernel, kThreads,
          bytes + sizeof(Barrier)));
      const int maximum = blocks_per_sm * property.multiProcessorCount;
      for (int contexts : {1, 2, 4, 8, 16, 32}) {
        if (contexts > maximum) continue;
        MapAllocation maps{};
        if (path == kTensor) {
          maps = make_maps(direction == kG2S ? source : destination, bytes,
                           contexts, map_mode, maps_to_free);
          warm_maps(maps);
        }
        CUDA_CHECK(cudaMemset(samples, 0, contexts * kRepeats * sizeof(Sample))); CUDA_CHECK(cudaMemset(destination, 0, static_cast<size_t>(kRepeats) * contexts * bytes));
        void* arguments[] = {&path, &direction, &map_mode, &bytes, &maps.pointer, &source, &destination, &begins, &issue_ends, &ends, &samples};
        CUDA_CHECK(cudaLaunchCooperativeKernel(reinterpret_cast<void*>(cooperative_same_kernel), contexts, kThreads, arguments, bytes + sizeof(Barrier))); CUDA_CHECK(cudaDeviceSynchronize());
        CUDA_CHECK(cudaMemcpy(host.data(), samples, contexts * kRepeats * sizeof(Sample), cudaMemcpyDeviceToHost)); std::vector<unsigned long long> hb(contexts * kRepeats), hi(contexts * kRepeats), he(contexts * kRepeats); CUDA_CHECK(cudaMemcpy(hb.data(), begins, hb.size() * 8, cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(hi.data(), issue_ends, hi.size() * 8, cudaMemcpyDeviceToHost)); CUDA_CHECK(cudaMemcpy(he.data(), ends, he.size() * 8, cudaMemcpyDeviceToHost));
        std::vector<Sample> aggregate(kRepeats); for (int repeat = 0; repeat < kRepeats; ++repeat) { auto begin_it = hb.begin() + repeat * contexts, issue_it = hi.begin() + repeat * contexts, end_it = he.begin() + repeat * contexts; const auto first_begin = *std::min_element(begin_it, begin_it + contexts); aggregate[repeat].issue_global_ns = *std::max_element(issue_it, issue_it + contexts) - first_begin; aggregate[repeat].global_ns = *std::max_element(end_it, end_it + contexts) - first_begin; for (int block = 0; block < contexts; ++block) { const Sample& row = host[repeat * contexts + block]; aggregate[repeat].issue_cycles = std::max(aggregate[repeat].issue_cycles, row.issue_cycles); aggregate[repeat].total_cycles = std::max(aggregate[repeat].total_cycles, row.total_cycles); aggregate[repeat].errors += row.errors; } }
        ...
        emit("same_direction", "multi_cta", path, direction_name(direction), "na", kBatched, map_mode, bytes, contexts, contexts, aggregate, errors, path == kTensor && direction == kG2S ? &maps : nullptr, path == kTensor && direction == kS2G ? &maps : nullptr);
      }
    }
  } else {
