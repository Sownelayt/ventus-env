#!/usr/bin/env python3
"""Audit Ventus TMA V3.4 generated RTL before expensive DC synthesis.

Background:
  V3.4 replaces 24 wide TransferSlots with 40 narrow LineContexts and six
  physical 128-byte PayloadSlots. Register-bit reduction alone is not enough:
  the generated RTL must also avoid duplicated payload banks, response
  broadcast networks, PMU state, and runtime divide/remainder hardware.
...
Usage:
  python3 benchmarks/tma-area-dc-v3.4/analyze_rtl_structure.py
  python3 benchmarks/tma-area-dc-v3.4/analyze_rtl_structure.py \
    --rtl gpgpu/generated-tma-v2-v3.4/tma.sv --output report.json
...
DEFAULT_RTL = ROOT / "gpgpu/generated-tma-v2-v3.4/tma.sv"
...
def main() -> None:
    ...
    planner_queue_bits = sum(
        ...
        if name.startswith("Queue1_TmaV2CursorExpandToken")
    ...
    engine_reduction_percent = (
        1.0 - engine_bits / baseline_engine_bits
    ) * 100.0
    ...
    gates = {
        ...
        "window_engine_le_23161": engine_bits <= 23161,
        "standalone_le_37480": standalone_bits <= 37480,
        ...
        "planner_queues_le_5000": planner_queue_bits <= 5000,
        "binder_le_384":
            audit["subtree_bits"]["TmaV2CommandBinder"] <= 384,
        "ingress_le_1500":
            audit["own_bits"]["TmaV3Ingress"] <= 1500,
        "descriptor_service_le_3000":
            audit["subtree_bits"]["TmaV2DescriptorService"] <= 3000,
    ...
    if not result["all_gates_passed"]:
        raise SystemExit("V3.4 structural gate failed")
