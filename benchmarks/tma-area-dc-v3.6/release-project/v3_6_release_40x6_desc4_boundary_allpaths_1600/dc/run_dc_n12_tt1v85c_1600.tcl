# Standalone Ventus TMA V3.6 all-path synthesis at TSMC N12 TT 1.0 V 85 C.
#
...
set synth_freq_mhz [getenv_default "SYNTH_FREQ_MHZ" 1600.0]
set report_freq_mhz [getenv_default "REPORT_FREQ_MHZ" 1600.0]
...
set variant [getenv_default \
  "VARIANT" "v3_6_release_40x6_desc4_boundary_allpaths_1600"]
...
puts "Ventus TMA V3.6 all-path DC: $variant top=$design_name"
...
# Do not let the single worst endpoint monopolize optimization.  First split
# the four architectural path classes, then give each material TMA sub-block
# its own register-endpoint group.  Paths which end outside a named sub-block
# remain in REG_TO_REG.  Every group is checked independently after mapping.
...
if {[sizeof_collection $all_regs] > 0} {
  group_path -name REG_TO_REG -from $all_regs -to $all_regs \
    -critical_range $critical_range -weight 2.0
}
if {[sizeof_collection $data_inputs] > 0 && \
    [sizeof_collection $all_regs] > 0} {
  group_path -name INPUT_TO_REG -from $data_inputs -to $all_regs \
    -critical_range $critical_range -weight 2.0
}
if {[sizeof_collection $all_regs] > 0 && \
    [sizeof_collection $outputs] > 0} {
  group_path -name REG_TO_OUTPUT -from $all_regs -to $outputs \
    -critical_range $critical_range -weight 2.0
}
if {[sizeof_collection $data_inputs] > 0 && \
    [sizeof_collection $outputs] > 0} {
  group_path -name INPUT_TO_OUTPUT -from $data_inputs -to $outputs \
    -critical_range $critical_range -weight 2.0
}

foreach group_spec [list \
    [list INGRESS "core/ingress/*"] \
    [list PLANNER "core/ingress/windowPlanner/*"] \
    [list BINDER "core/ingress/binder/*"] \
    [list DESCRIPTOR "core/subsystem/descriptor/*"] \
    [list ENGINE "core/subsystem/engine/*"]] {
  set group_name [lindex $group_spec 0]
  set group_pattern [lindex $group_spec 1]
  set endpoint_regs [filter_collection \
    $all_regs "full_name =~ $group_pattern"]
  if {[sizeof_collection $endpoint_regs] > 0} {
    group_path -name $group_name -to $endpoint_regs \
      -critical_range $critical_range -weight 3.0
  } else {
    puts "WARNING: no register endpoints found for $group_name"
  }
}
...
foreach group_name [list \
    ...
    INGRESS PLANNER BINDER DESCRIPTOR ENGINE] {
...
puts $summary "Path_Groups: REG_TO_REG INPUT_TO_REG REG_TO_OUTPUT INPUT_TO_OUTPUT INGRESS PLANNER BINDER DESCRIPTOR ENGINE"
...
puts "INFO: TMA V3.6 N12 TT 1.0V 85C 1.6GHz all-path synthesis finished"
exit
